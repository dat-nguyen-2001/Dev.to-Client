...
const likeArticle = async(articleId: number) => {
    await axios.post('http://devtobackend.herokuapp.com/likes', {articleId}, {headers: {Authorization: `Bearer ${sessionStorage.getItem('accessToken')}`}})
}

const getLikedArticles = async() => {
    const res = await axios.get('http://devtobackend.herokuapp.com/likes', {headers: {Authorization: `Bearer ${sessionStorage.getItem('accessToken')}`}})
    return res.data
}

const commentOnArticle = async(content: string, articleId: number) => {
    await axios.post('http://devtobackend.herokuapp.com/comments', {content, articleId}, {headers: {Authorization: `Bearer ${sessionStorage.getItem('accessToken')}`}})
}
